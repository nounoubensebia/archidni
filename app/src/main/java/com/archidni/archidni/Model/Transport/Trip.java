...
public class Trip {
    private long days;
    private ArrayList<Pair<Station,Integer>> stationTimes;


    public Trip(long days, ArrayList<Pair<Station, Integer>> stationTimes) {
        this.days = days;
        this.stationTimes = stationTimes;
    }

    public ArrayList<Pair<Station, Integer>> getStationTimes() {
        return stationTimes;
    }
    ...
    public Station getOrigin (Line line)
    {
        ...
        int minId = stationTimes.get(0).first.getId();
        for (int i = 1; i < stationTimes.size(); i++) {
            if (min>stationTimes.get(i).second)
            {
                min = stationTimes.get(i).second;
                minId = stationTimes.get(i).first.getId();
            }
        }
